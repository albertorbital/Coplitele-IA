...
// 1. Replace the existing section-nav-grid in the Inicio section safely using regex that bounds on the very specific HTML.
// We know it starts with <div class="section-nav-grid"> and ends with </a>\n        </div>\n      </div>\n    </section>
html = html.replace(/<div class="section-nav-grid">[\s\S]*?<a href="#\/impacto#recursos"[\s\S]*?<\/a>\s*<\/div>/, newGridHTML);


// 2. Inject the SECOND grid AFTER "Últimas noticias" section and BEFORE "Actividades de Transferencia" section in Impacto.
const newsSectionEnd = html.indexOf('</section>', html.indexOf('<section id="noticias"'));
const beforeTransSection = html.indexOf('<!-- Section 2: Actividades de Transferencia -->', newsSectionEnd);
html = html.substring(0, beforeTransSection) + 
       `\n    <!-- Nav Grid Impacto -->\n    <div class="section-container" style="margin-top: 40px; margin-bottom: 20px;">\n` + 
       newGridHTML + 
       `\n    </div>\n\n    ` + 
       html.substring(beforeTransSection);
